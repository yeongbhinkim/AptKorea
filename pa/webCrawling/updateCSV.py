...
class CsvImporter:
    ...
    def preprocess_csv(self, input_path, output_path):
        with open(input_path, 'r', newline='', encoding='utf-8') as infile, \
            ...
            # Copy header and add 'FULL_CONTRACT_DATE'
            header = next(reader)
            header.append('FULL_CONTRACT_DATE')
            ...
            for row in reader:
                ...
                # Create 'FULL_CONTRACT_DATE' value
                contract_day = row[7].zfill(2)  # Assuming 'CONTRACT_DAY' is the 8th column
                full_contract_date = row[6] + contract_day  # Assuming 'CONTRACT_DATE' is the 7th column
                row.append(full_contract_date)
    ...
    def import_csv_files(self):
        ...
        for csv_file in csv_files:
            ...
            mysql_command = [ 
                ...
                '-u', db_user,
                '-p' + db_pass,
                '-e', f"LOAD DATA LOCAL INFILE '{processed_path.replace('\\', '\\\\')}' INTO TABLE {db_name}.{table_name} FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\\n' IGNORE 1 LINES (CITY, STREET, BON_BUN, BU_BUN, DAN_GI_MYEONG, SQUARE_METER, CONTRACT_DATE, CONTRACT_DAY, AMOUNT, LAYER, CONSTRUCTION_DATE, ROAD_NAME, REASON_CANCELLATION_DATE, REGISTRATION_CREATION, TRANSACTION_TYPE, LOCATION_AGENCY, FULL_CONTRACT_DATE);"
            ]
